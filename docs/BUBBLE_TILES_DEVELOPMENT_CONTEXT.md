...
- The local working tree is the authoritative v1.8.1 patch candidate.
- Repository HEAD contains the committed v1.8 release.
- The local v1.8.1 work should not be committed, pushed, published, or deployed unless the user explicitly asks. The user always performs the commit themselves.
- The browser title, About dialog, Help heading, README, and Version History identify the current work as v1.8.1. The prominent header wordmark intentionally shows only the minor release, v1.8; future patch releases such as v1.8.2 should retain v1.8 in that corner branding.
...
The detailed application version is v1.8.1 in the current files, and Version History is shown in the modal content in [index.html](../index.html). The prominent header wordmark uses the minor version only (v1.8) for patch releases. Detailed version displays and historical entries should remain consistent when release-version changes are requested.
