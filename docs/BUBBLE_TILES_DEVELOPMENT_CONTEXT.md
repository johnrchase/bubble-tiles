...
- The local working tree is the authoritative v1.9.1 patch candidate.
- Repository HEAD contains the committed v1.9 release.
- The local v1.9.1 work should not be committed, pushed, published, or deployed unless the user explicitly asks. The user always performs the commit themselves.
- The browser title, About dialog, Help heading, README, and Version History identify the current work as v1.9.1. The prominent header wordmark intentionally remains at the minor release, v1.9.
- Version 1.6 includes a live bite/bump counter; editable lattice vectors; single-tile custom image fills with upload, clipboard paste, drag-and-drop, mouse-wheel resizing, anonymous paginated Openverse search and supported media-type filtering; clipped tile text with adjustable font, size, color, bold, and italic styling; corrected view-scaling canvas borders with rounded joins and caps; and an image-filled cloud tiling example. Custom Image and Add Text are available from both the Style toolbar and Style dropdown for non-frame tiles. Frames accept solid fill colors, opacity, and outline styling but reject patterns, custom images, and text. New text is screen-horizontal when applied and follows subsequent tile transforms. Image and text styling persist through JSON, duplication, lattice rendering, sharing, and picture export.
- The committed v1.6.1 patch combines lattice creation and editing in Tiling Fill, prevents duplicate lattices for one source set, restores reliable donut previews and picture exports, adds the Bubble Bubble Tiles example, supports mouse-wheel custom-image resizing, limits frames to solid-color fills, and includes Soft Tiles in bite-and-bump totals.
- Version 1.7 corrects the hexagonal H3B/H3C identities: H3B is the alternating three-bite form in orange-gold `#f6b42e`, while H3C is the three-consecutive-bite form in brown-orange `#da9640`. Their puzzle-style counterparts follow the same identities. Stored examples and frame definitions are migrated so their geometry remains unchanged. Soft Tile 3B/3C keep their existing labels and geometry but swap colors only: Soft Tile 3B is brown-orange `#da9640`, and Soft Tile 3C is orange-gold `#f6b42e`.
- Version 1.8 centers lattice candidate generation on the current viewport rather than always around the source motif, allowing ordinary loaded and panned views to render periodic copies through every visible edge while retaining the existing tile budget. It also keeps the Bubble Style, Fill, Outline, Vertices, and Decompose popovers inside narrow browser windows; adds optional per-tile vertex points with adjustable color and size plus unit-interval decomposition along whole-number edge lengths; and adds a Bubble-toolbar Decompose tool that replaces compatible hexagonal, trapezoidal, rhombic, or Soft Tiles with grouped, style-preserving pieces while retaining original outer arcs and adding complementary shared internal arcs. Soft Tiles are treated as an H0 core with one attached T3 per point, and can decompose into H0 plus T3 tiles, all triangles, triangles with rhombuses, or triangles with trapezoids. Version 1.8 also refines the Outline icon and dropdown spacing and uses pure white for new canvas backgrounds while preserving colors explicitly stored in layouts.
- Version 1.8.1 adds the native Rhombic chiral partners 1*, 2A*, and 3*, corrects Rhombic names and family colors, and updates Arc Dual colors for the chiral pairs. Horizontal and vertical reflection switch the displayed identity and default color of the Hexagonal 3A/3A* pair (including puzzle-style variants) and the Rhombic 1/1*, 2A/2A*, and 3/3* pairs while preserving the exact reflected geometry and any customized fill styling. Edge-preserving Rhombic Arc-Dual variants retain a canonical chiral identity so subsequent H/V reflections also switch to the correct partner color and identity without moving arcs to different physical edges. The patch also adds a preview-free Paper Figures gallery with stable descriptive slugs, direct `?paper=` URLs, and right-click share-link copying.
- Version 1.9 adds a browser-native warning before closing, reloading, or navigating away from a layout with unsaved content changes; zoom and pan alone do not count as changes. Saving to browser, exporting JSON, copying a share link, or loading an existing layout establishes a clean baseline. A single recognized tile selection reports its concise mathematical name in the status bar, including the H, P, R, S, and T family prefixes, while custom and multiple selections retain a tile count. Reverse One Arc preserves the tile’s visible fill color when it generates an edited arc variant. One or more selected H3A/H3A* tiles can toggle scalable connected-arrow markings from the right-click menu, while all non-image tiles can toggle vivid rainbow orientation coloring from the Fill controls; both states follow transforms, copying, JSON/share persistence, lattice fills, and picture export. Newly placed H3A and H3A* tiles use the paper’s preferred reference orientations. Copy retains editable tiles in the app's internal clipboard and also writes a transparent PNG of the selected tiles to the system clipboard when the browser supports image clipboard access. The About dialog places a feedback invitation above a concise resource list with the working paper, Tim Reinhardt’s iOS beta game, Alex McCluer’s computational One-of-each solution, the repository, and MathHappens credit. The release also replaces Paper Figures 3 and 26 and the Hexagonal, Triangular, Square, Soft Tile, and Rhombic Add All reference arrangements with revised source layouts; substantially expands the directly linkable Paper Figures; and increases the bounded lattice overscan margin so ordinary panning and zooming is less likely to expose a narrow gap at a viewport edge.
- Version 1.9.1 adds further interactive paper figures, groups them by the current paper sections, shifts subsequent display labels to match the revised paper order while preserving stable slugs, refreshes the complete Paper Figure URL manifest in exact display order, and preserves H3A arrow decorations in downloaded picture exports.
- Every Tiles-tray family heading has a concise native hover description of the family's underlying geometry or purpose; Help directs users to hover over a family heading before opening it.
- The Examples tray includes `soft-rhombic-triangular-tiling.json`, a 25-tile periodic seed combining Soft Tile S6, Rhombic R2B, and Triangular T0/T2/T3 tiles.
- The Examples tray includes `small-frame-lattice.json`, a seven-tile periodic extension of a Small frame solution using H1, H4B, and H6.
- The Examples tray includes `six-point-star-puzzle-tiling.json`, a four-tile periodic seed combining a bubble-converted six-point star with three T0 triangular bubble tiles.
- The Examples tray includes `soft-sf0-sf3a-lattice.json`, a three-tile periodic seed combining one SF0 with two SF3A Soft Tiles.
- The Examples tray includes `hex-pac-man-and-triangles.json`, a ten-tile periodic seed combining six Hex Pac-Man tiles with four regular triangles under Other Infinite Tilings.
- The Examples tray includes `trapezoidal-tiling.json`, a six-tile periodic seed using complementary trapezoidal bubble tiles with the Spike Bubble Style under Infinite Bubble Tilings. Puzzle Squiggle Octagon Tiling and Arc Squiggle Octagon Tiling are also categorized under Infinite Bubble Tilings.
- Examples are ordered by mathematical theme: finite frame solutions; complete-family lattices; other infinite bubble tilings; infinite soft tilings; other periodic tilings; then an Other section containing finite spirals, Pac-Man, Spectre, tangram, Penrose, and Heesch investigations. The former “Regular Polygon Tiling” is identified as the semiregular `3.4.6.4` Rhombitrihexagonal Tiling.
- The Examples tray presents those themes as native expandable sections. Each section heading has a concise hover description; the Finite Frames section starts open and the remaining sections start collapsed.
- The Tiles and Examples tabs have instructional hover descriptions. The Examples tab tooltip carries the loading guidance instead of displaying a permanent instruction line inside the tray.
- The final Examples category is Paper Figures. It contains a second expandable level that groups figures by the current paper sections and appendices, followed by Additional Layouts for unnumbered companions. Its layouts are stored separately in `examples/paper-figures.js` and related figure assets, omit tray previews, and use stable descriptive slugs. Right-clicking a Paper Figure card offers Copy Share Link, producing a hosted URL in the form `https://johnrchase.github.io/bubble-tiles/?paper=<slug>`. On startup, the app resolves that query parameter to the matching built-in paper layout; full `#layout=` share hashes take precedence if both are present.
- Paper Figures that appear in the current paper draft are ordered by their first figure/panel reference and labeled `Fig N(letter)` in the tray. Frequency ratios render as semantic HTML subscripts while the underlying title remains plain accessible text. Figures not matched to the draft retain their existing names and follow the labeled entries; descriptive share-link slugs never include figure numbers.
...
The lower-right status bar reports global bite and bump totals for placed canvas tiles whose definitions contain bubble or puzzle-edge directions. For Soft Tiles, each point contributes two bites and each non-point contributes one bump, treating the tile as an H0 tile with T3 tiles attached at its points. Polygonal tiles, frames, pasted images, and virtual lattice copies do not contribute. The count follows changes such as adding, deleting, loading, Arc Dual, and Reverse One Arc.
...
### Border styles
The app supports stroke color, stroke style (solid, dashed, dotted, or none), and stroke width. These are exposed through the Style UI and preview grids. The Vertices control is off by default and can render adjustable-color, adjustable-size points at a tile definition's vertices. Its Integral decomposition option inserts additional marks at unit intervals when an underlying straight edge is a whole-number multiple of `SIDE_LENGTH`; state follows copying, JSON/share serialization, lattice copies, and picture export. Live canvas and lattice strokes scale with the SVG view and tile transforms; tray thumbnails retain their separate compact non-scaling strokes. Rounded joins and per-edge caps avoid miter spikes at curved vertices. Shared-border suppression compares sampled transformed edge profiles, not just common endpoints, so distinct inward/outward curves that enclose a lens retain both outlines.
...
The detailed application version is v1.9.1 in the current files, and Version History is shown in the modal content in [index.html](../index.html). The prominent header wordmark uses the minor version only (v1.9) for patch releases. Detailed version displays and historical entries should remain consistent when release-version changes are requested.
...
9. Load the Donut Trio example with a non-white canvas and verify that sprinkle, chocolate, and glazed donut images all render on the canvas, throughout the Export Picture preview, and in the downloaded PNG. Confirm that preview letterboxing retains the selected canvas color and that no donut image-fill type disappears during raster export.
